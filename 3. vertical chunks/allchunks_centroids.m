...
for i = 1:length(backbones)
    ...
    loc1 = readtable(cat(2,folder,num2str(backbones(i)),'.csv')); % geolocations of backbone
    loc2 = readtable(cat(2,folder,num2str(additions(i)),'.csv')); % geolocations of strip being added
    ...
    writetable(loc,cat(2,output_path,num2str(output_files(i)),'.csv'))
